...
三级功耗策略，由 `PowerManager` 任务每 500ms 轮询管理：
...
射频波唤醒 (PA0 WKUP)
  ...
  │                            │
  ▼                            │
ACTIVE ──(5s 无活动)──→ DIM ──(30s 无活动 + 非扫描)──→ DEEP_SLEEP (STANDBY)
  ↑                      ↑
  └── 收到指令/判决数据 ──┘      (refreshActivity 重置计时器)
```

| 状态 | OLED | 蓝牙 | 采样 | 说明 |
|------|------|------|------|------|
| ⚡ ACTIVE | 常亮 | 正常收发 | 持续 | 全功能运行 |
| 💤 DIM | 熄灭 | 正常收发 | 持续 | 仅关闭 OLED，采样和蓝牙保持工作 |
| 😴 DEEP_SLEEP | 断电 | 断电 | 停止 | STANDBY 模式，PA0 检测射频波唤醒 |

### 自主唤醒机制

进入 DEEP_SLEEP 前，系统将 PA0 配置为 WKUP1 唤醒引脚。当射频能量检测电路在 PA0 上产生上升沿信号时，MCU 从 STANDBY 模式复位唤醒，重新执行 `main()` → 各外设初始化 → `DefaultTask` 创建任务，系统回到 ACTIVE 状态开始新一轮波束扫描和解调。
