...
void StartDefaultTask(void *argument)
{
  ...
  /* Infinite loop */
	...
	// 创建信息处理任务，实时性要求偏低
	messageTaskCreateReturn = xTaskCreate(messageTask, "messageTask", 128, NULL, osPriorityBelowNormal, &messageTaskHandle);
...
void messageTask(void *params)
{
    ...
    QueueSetMemberHandle_t activatedMember = xQueueSelectFromSet(masterQueueSet, portMAX_DELAY);
			...
			if (osMessageQueueGet(blueToothQueueHandle, &pReceivedPtr, NULL, 0) == osOK)
      {
				...
				// 3. 温度读取指令: "TEM"
				else if (strncmp((char*)pReceivedPtr, "TEM", 3) == 0)
				{
					Power_RefreshActivity();
					if (DS18B20_Init() == 0) {
						short tem = DS18B20_Get_Temp(); // 此函数内部有 750ms 延时，放在这里跑最安全
						BT_SendPrintf_DMA("Temp:%d.%d", tem/10, abs(tem%10));
					}else{
						BT_SendPrintf_DMA("Temp:InitError");
					}
				} 
				// OLED 显示
		...
		else if(activatedMember == (QueueSetMemberHandle_t)messageQueueHandle)	// 解调消息处理
		{
			if (osMessageQueueGet(messageQueueHandle, &pReceivedPtr, NULL, 0) == osOK)
			{
				...
				/* 信息处理状态机 */
				// 1.返回温度信息
				if(strncmp((char*)pReceivedPtr, "TEM", 3) == 0 && strlen((char*)pReceivedPtr) == 3)
				{
